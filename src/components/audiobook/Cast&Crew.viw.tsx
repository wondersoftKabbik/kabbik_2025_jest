      ...
      <div className="max-w-2xl mx-auto">
        <div className="flex flex-col gap-[27px]">
          {castCrewData?.map((author) => (
            <div
              onClick={()=>router.push(paths.cast+author.name)}
              key={author.id}
              className=" w-full rounded-xl cursor-pointer bg-white shadow-[0_1px_4px_1px_rgba(0,0,0,0.25)] flex items-center px-[15px] py-[7px]"
            >
              {/* Profile Image */}
